    ...
    <q-page padding>
        <div class="q-pa-md">
            ...
            <q-toolbar class="shadow-1 rounded-borders q-mb-lg">
                <q-btn flat :label="$t('create') + ' System Parameter'" icon="add" color="white" class="bg-primary"
                    @click="openCreateDialog" />
                <q-space />
                <div class="col-6">
                    <q-input dense standout="bg-primary" v-model="searchText" input-class="search-field text-left"
                        class="q-ml-md" placeholder="Search by name or value...">
        ...
        <q-dialog v-model="showDialog" persistent>
            <q-card style="min-width: 500px">
                <q-card-section>
                    <div class="text-h6">{{ isEditMode ? 'Edit System Parameter' : 'Create System Parameter' }}</div>
                </q-card-section>
        ...
        <q-dialog v-model="showDeleteDialog" persistent>
            <q-card>
                ...
                <q-card-actions align="right">
                    ...
                    <q-btn flat label="Delete" color="negative" @click="deleteParameter" :loading="deleting" />
                </q-card-actions>
            </q-card>
        </q-dialog>
    </q-page>
...
// Methods
const fetchParameters = async (paginationData = pagination.value) => {
    loading.value = true
    try {
        const params = {
        ...
        const response = await api.get('/api/system-parameters/paginated', { params })
...
const onRequest = (props) => {
    ...
    fetchParameters(pagination.value)
}
...
const saveParameter = async () => {
    ...
    try {
        let response
        if (isEditMode.value) {
            response = await api.put(`/api/system-parameters/${formData.value.id}`, formData.value)
        } else {
            response = await api.post('/api/system-parameters', formData.value)
        }

        if (response.data.success) {
            $q.notify({
                type: 'positive',
                message: isEditMode.value ? 'Parameter updated successfully' : 'Parameter created successfully'
            })
            closeDialog()
...
const deleteParameter = async () => {
    deleting.value = true
    try {
        const response = await api.delete(`/api/system-parameters/${itemToDelete.value.id}`)
        if (response.data.success) {
            $q.notify({
                type: 'positive',
